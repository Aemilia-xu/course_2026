# 《生成模型原理与应用》课程作业
本项目基于 Stable Diffusion，实现了一个 DreamBooth + LoRA 的个性化图像生成系统。

通过对少量自定义实例图片进行 参数高效微调（LoRA），模型能够学习并记住特定对象或风格（如某只猫、某个角色或特定物体），并在保持原有生成能力的同时，根据文本提示生成高质量、风格一致的图像。

项目包含完整的训练流程与推理流程，支持在有限计算资源下快速完成定制化模型训练，适用于个性化图像生成。
